...
namespace 单纯形法
{
    ...
    class matrix
    {
        ...
        public matrix(int m_rows, int m_cols, string m_input)
        {
            ...
            if (res.Length != m_rows * m_cols)
                throw new Exception($"解析字符串获取的数据数量为{res.Length},无法生成行数:{m_rows},列数:{m_cols}的矩阵");
            this.rows = m_rows;
            this.cols = m_cols;
            this.elems = new Retional[rows, cols];
            short count = 0;
    ...
    class DanChunXingSolver
    {
        ...
        private matrix CB;      // CB
        private matrix CN;      // CN
        private matrix N;       // 非基变量对应的矩阵
        ...
        private string showproblem()
        {
            ...
            {
                res += $"\n\t{m_table[i + 1, 1].GetResult()}\t=";
                for (int j = 1; j <= m_A.Cols; j++)
                {
                    if (m_A[i, j].IsPositive())
                        if (j == 1)
                            res += $"{m_A[i, j].GetResult()}x{j} ";
                        else
                            res += $"+ {m_A[i, j].GetResult()}x{j} ";
